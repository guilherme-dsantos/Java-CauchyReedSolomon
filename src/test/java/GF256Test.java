...
class GF256Test {
    ...
    void testAdd() {
        ...
        for (int a = 0; a < 256; a += 13) {
            for (int b = 0; b < 256; b += 17) {
                assertEquals(
                        gf256.add((byte)a, (byte)b),
                        gf256.add((byte)b, (byte)a)
                );
            }
        }
    ...
    void testInitFailureInPolyInit() {
        ...
        doThrow(new RuntimeException("Polynomial initialization failed"))
                .when(gf256).polyInit(anyInt());
    ...
    void testInitFailureInExpLogInit() {
        ...
        doThrow(new RuntimeException("Exp/Log initialization failed"))
                .when(gf256).expLogInit();
    ...
    void testInitFailureInMulDivInit() {
        ...
        doThrow(new RuntimeException("Mul/Div initialization failed"))
                .when(gf256).mulDivInit();
    ...
    void testInitFailureInInvInit() {
        ...
        doThrow(new RuntimeException("Inverse initialization failed"))
                .when(gf256).invInit();
