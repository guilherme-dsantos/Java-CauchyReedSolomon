...
class CodingTests {
    ...
    public static List<List<Integer>> getCombinationIndices(int n, int m) {
    ...
    void testInvalidParameters() {
        ...
        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(0, 1, new byte[1][8], new byte[8], 8), "Should throw InvalidParametersException with k=0");

        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(-5, 1, new byte[1][8], new byte[8], 8), "Should throw InvalidParametersException with negative k");

        // Test with invalid m values
        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(1, 0, new byte[1][8], new byte[1], 8), "Should throw InvalidParametersException with m=0");

        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(1, -3, new byte[1][8], new byte[1], 8), "Should throw InvalidParametersException with negative m");

        // Test with k+m exceeding 256
        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(250, 7, new byte[250][8], new byte[7 * 8], 8), "Should throw InvalidParametersException with k+m>256");

        // Test with invalid block sizes
        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(4, 2, new byte[4][7], new byte[2 * 7], 7), "Should throw InvalidParametersException with blockBytes not multiple of 8");

        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.encode(4, 2, new byte[4][8], new byte[2 * 8], -8), "Should throw InvalidParametersException with negative blockBytes");

        // Test with null pointers
        assertThrows(CauchyException.NullDataException.class, () -> Cauchy256.encode(4, 2, null, new byte[2 * 8], 8), "Should throw NullDataException with null data pointers");

        assertThrows(CauchyException.NullDataException.class, () -> Cauchy256.encode(4, 2, new byte[4][8], null, 8), "Should throw NullDataException with null recovery blocks");

        // Test decode with invalid parameters
        assertThrows(CauchyException.InvalidParametersException.class, () -> Cauchy256.decode(0, 2, new Cauchy256.Block[2], 8), "Should throw InvalidParametersException with k=0 in decode");

        assertThrows(CauchyException.NullDataException.class, () -> Cauchy256.decode(4, 2, null, 8), "Should throw NullDataException with null blocks array in decode");
    }

    @Test
    @DisplayName("Test with dataset")
    void datasetCodingTest() {

        int[][] configs = {{2, 2}, // k=2 m=2
                {3, 1}, // k=3 m=1
        };
        ...
        for (int[] km : configs) {
            ...
            for (File f : sampleFiles) {
                try {
                    ...
                    // Encode data
                    assertDoesNotThrow(() -> Cauchy256.encode(k, m, dataBlocks, recoveryBlocks, blockBytes), "Encoding with k=" + k + ", m=" + m + " should succeed");
    ...
    void testDecodeNoMissingBlocks(int k, int m, byte[][] dataBlocks, int blockBytes) {
        ...
        assertDoesNotThrow(() -> Cauchy256.decode(k, m, blocks, blockBytes), "Decode should succeed with no missing blocks");

        // Verify all data matches the original
        for (int i = 0; i < k; i++) {
            assertArrayEquals(originalData[i], blocks[i].data, "Block " + i + " data should be unchanged after decoding");
        }
    }
    ...
    void testDecodeTooManyMissingBlocks() {
        ...
        assertThrows(CauchyException.InsufficientBlocksException.class, () -> Cauchy256.decode(k, m, blocks, blockBytes), "Decode should fail with too many missing blocks");
    }

    private void testRecoveryWithAllPossibleLossCombinations(byte[][] dataBlocks, byte[] recoveryBlocks, int blockBytes, int k, int m) {
        System.out.println("Starting test with parameters: k=" + k + ", m=" + m + ", blockBytes=" + blockBytes);

        // Test all loss scenarios from 1 to m blocks
        for (int lossCount = 0; lossCount <= m; lossCount++) {
            ...
            System.out.println("Generated " + lossCombinations.size() + " combinations for loss count " + lossCount);
            ...
            for (List<Integer> combination : lossCombinations) {
                ...
                System.out.println("\n[Combo " + combinationCounter + "/" + lossCombinations.size() +
                        "] Testing loss combination: " + combination);
                ...
                System.out.println("  - Lost data blocks: " + lostDataBlockCount +
                        ", Lost recovery blocks: " + lostRecoveryBlockCount);
                ...
                assertDoesNotThrow(() -> {
                    ...
                    System.out.println("  - Decode SUCCESSFUL with " + finalLostDataCount + " lost data blocks");

                    // Print blocks array after decode for debugging
                    System.out.println("  - Blocks array after decode:");
                    for (int i = 0; i < blocks.length; i++) {
                        if (blocks[i] != null) {
                            System.out.println("    - Block[" + i + "]: row=" + blocks[i].row +
                                    ", data length=" + blocks[i].data.length);
                        } else {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (Cauchy256.Block block : blocks) {
                if (block != null && block.row == i) {
                    assertArrayEquals(originalData[i], block.data, "Recovered block " + i + " should match original");
                    blockFound = true;
                    break;
                }
            }
